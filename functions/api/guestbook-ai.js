...
const SYSTEM_PROMPT = `你是武汉理工大学资源分享网站的留言板AI助手，负责分析留言并决定处理方式。所有输出必须是纯文本，禁用Markdown。
    ...
    4. 禁止封禁【管理员】
    【内容识别】
    - 礼貌性检测：若留言为资源请求且完全缺失"请"、"求"、"麻烦"、"谢"等礼貌词汇（如直接发"高数试卷"、"我要大物"） -> reject_message(这是公益站，求资源请注意礼貌)
...
export async function processWithAIAgent(guestbookEntry, env, autoMode) {
    ...
    const systemPromptToUse = autoMode
        ? SYSTEM_PROMPT + `\n\n【自动审核模式】当前为自动审核模式，你需要检查内容是否合规，并尝试搜索资源。
            ...
            2. 礼貌性检查 -> 若资源请求缺失"请"、"求"等礼貌词，直接驳回(reject_message: 公益站请讲礼貌)
            3. 模糊/不完整请求 -> 仍需驳回（如：仅课程名无类型、表述不清、含无关内容）
            4. 纯粹感谢/祝福 -> 使用 mark_resolved 直接回复（填写 reply 和 note），无需搜索。
            5. 表述清晰完整的资源请求（含具体课程名+资源类型）-> 使用 search_resources 搜索资源
            6. 如果搜索到匹配资源（通过二次调用判断） -> 使用 mark_resolved 标记为已解决，必须提供 matched_file_index，同时在 note 中填写版本/年份等说明。
            7. 如果未搜索到资源 -> keep_pending 等待人工处理
